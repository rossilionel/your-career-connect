...
export class EventDetailPage {
  ...
  share(event, type){
  	switch(type){
  		case 0: 
  			this.socialSharing.shareViaFacebook(this.event.name, "", 'http://yourcareerconnect.com/events/detail/index.php?id='+ this.event.id).then(() => {
			...
			break;
		case 1: 
  			this.socialSharing.shareViaTwitter(this.event.name, "../assets/share.png", 'http://yourcareerconnect.com/events/detail/index.php?id='+ this.event.id).then(() => {
			...
			// this.emailComposer.isAvailable().then((available: boolean) =>{
			 // if(available) {
			   let email = {
				  ...
				  body: 'http://yourcareerconnect.com/events/detail/index.php?id='+ this.event.id,
				  isHtml: true
				};
