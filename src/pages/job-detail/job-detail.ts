...
export class JobDetailPage {
  ...
  share(event, type){
  	switch(type){
  		case 0: 
  			this.socialSharing.shareViaFacebook(this.job.title, "", 'http://yourcareerconnect.com/careers/detail/index.php?id='+ this.job.id).then(() => {
			...
			break;
		case 1: 
  			this.socialSharing.shareViaTwitter(this.job.title, "../assets/share.png", 'http://yourcareerconnect.com/careers/detail/index.php?id='+ this.job.id).then(() => {
			...
			// this.emailComposer.isAvailable().then((available: boolean) =>{
			 // if(available) {
			   let email = {
				  ...
				  body: 'http://yourcareerconnect.com/careers/detail/index.php?id='+ this.job.id,
				  isHtml: true
				};
